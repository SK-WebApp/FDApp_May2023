...
// This creates global variables storedUI1(user item input), 2 (Drink), 3(Food), 4a (filtered array#), 4b (filtered array description), 5(ItemType), 6(IDDSILev)
	...
	var storedUI5 = [];
	console.log(storedUI5)
	var storedUI6 = [];
	console.log(storedUI6)		
...
/* This sets up a function for displaying additional comment/instruction in the comment div. */
	...
	/* might not need this next part about extractedLevel and extractedDes, redundant code */	
		...
		if (carriedDFValue == "Drink" && carriedUI2 != "Do Not Know") {
			...
			if (currentLevel == "level2a" && extractedLevel < DlevelNumber) { 			
				htmlString += '<p>' + "You item doesn't match recommendation. " + "Your recommended <span> IDDSI drink level is " + carriedUI2 + "</span>, but your self-selected <u> IDDSI drink level is " + extractedLevel + ", " + extractedDes + "</u>. You can modify your item by making it thicker" + '.</p>';
				htmlString += '<p>' + "To increase swallowing safety, you can click on the Modification button to see how to adjust your self-selected food item to the recommended IDDSI drink level" + '.</p>';
				displayMod();
				}
			if (currentLevel == "level3a") {
			...
			if (currentLevel == "level3c" && extractedLevel < DlevelNumber) { 	
				htmlString += '<p>' + "Your item doesn't match recommendation. " + "Your recommended <span> IDDSI drink level is " + carriedUI2 + "</span>, but your self-selected <u> IDDSI drink level is " + extractedLevel + ", " + extractedDes + "</u>." + '</p>';
				htmlString += '<p>' + "To increase swallowing safety, you can click on the Modification button to see how to adjust your self-selected drink item to the recommended IDDSI food level" + '.</p>';
				displayMod();
				}
			if (currentLevel == "level4a") {
		...
		if (carriedDFValue == "Food" && carriedUI3 != "Do Not Know" ) {
			...
			if (currentLevel == "level2b" && extractedLevel == FlevelNumber && extractedDes != FDes)	{
				displayMod();
				htmlString += '<p>' + "Your item doesn't match recommendation. " + "Your recommended <span> IDDSI food level is " + carriedUI3 + "</span>, but your self-selected <u> IDDSI food level is " + extractedLevel + ", " + extractedDes + "</u>." + '</p>';
				htmlString += '<p>' + "To increase swallowing safety, you can click on the Modification button to see how to adjust your self-selected food item to the recommended IDDSI food level" + '.</p>';
				}
			if (currentLevel == "level2b" && extractedLevel > FlevelNumber) { 
				displayMod();
				htmlString += '<p>' + "Your item doesn't match recommendation. " + "Your recommended <span> IDDSI food level is " + carriedUI3 + "</span>, but your self-selected <u> IDDSI food level is " + extractedLevel + ", " + extractedDes + "</u>." + '</p>';
			...
			if (currentLevel == "level4b" && newValue == "mixed" && extractedLevel <= FlevelNumber) {
				displayMod();	
				htmlString += '<p>' + "Congratulations! You can consume your item." + "Your recommended <span> IDDSI food level is " + carriedUI3 + "</span>, and your self-selected <u> IDDSI food level is " + extractedLevel + ", " + extractedDes + "</u>. " + '.</p>';	
	...
	function RetrieveSelfSelectValue() {
		document.getElementById('userinput4').innerHTML = "Your self-selected IDDSI " + carriedDFValue + " Level is:  <span>" + carriedUI4a + ", "+ carriedUI4b + "</span>.";	

		if (carriedDFValue == "Transitional") { 
			document.getElementById('userinput4').innerHTML = "You selected your item type as:  <span>" + carriedDFValue + "</span>.";
			document.getElementById('userinput6').innerHTML = "You are currently exploring: " + carriedUI6 + " level " + carriedUI5 + " modification techniques.";
		}
	}
...
// 		alert ("display Caption for preparation videos")
		if (carriedDFValue == "Drink" && carriedUI2 != "Do Not Know") {
			document.getElementById('userinput5').innerHTML = "Click on each button to view preparation videos for  <span>" + carriedUI2 + "</span>.";
		}
		if (carriedDFValue == "Food" && carriedUI3 != "Do Not Know") {
			document.getElementById('userinput5').innerHTML = "Click on each button to view preparation videos for  <span>" + carriedUI3 + "</span>.";
		}
		if (carriedDFValue == "Transitional") {
			document.getElementById('userinput5').innerHTML = "Click on each button to view preparation videos for  <span>" + carriedUI6 + " " + carriedUI5 + "</span>.";		
		}	
	}
...
//   	 		alert("displayTest-transitional-food section")
	...
	function renderHTMLmodcomment() {
		...
		if (carriedDFValue == "Drink" && carriedUI2 != "Do Not Know") {
			...
			if (DlevelNumber[0] == "1") {
				htmlString += '<p>' + "IDDSI level " + DlevelNumber + " " + DDes + " can be drunk form a cup; has smooth texture with no lumps; doesn't require chewing or biting. <br />" + foodtext[DlevelNumber].ModificationNote +'</p>';		
			}	
			...
			if (DlevelNumber[0] == "4") {
				htmlString += '<p>' + "IDDSI level " + DlevelNumber + " " + DDes + " are typically too thick to be drunk form a cup or sucked through a straw; has smooth texture with no lumps; doesn't require chewing or biting. <br />" + foodtext[DlevelNumber].ModificationNote +'</p>';		
			}

			htmlString += '<p>' + "To qualify for IDDSI level " + DlevelNumber + " " + DDes + ", your " + carriedDFValue + " item must pass the following test(s). <br /> See video guide below for details" + '.</p>';
		}
		...
		if (carriedDFValue == "Food" && carriedUI3 != "Do Not Know" ) {
			...
			if (FlevelNumber[0] == "3") {
				htmlString += '<p>' + "IDDSI level " + FlevelNumber + " " + FDes + " can be drunk form a cup; has smooth texture with no lumps; doesn't require chewing or biting. <br />" + foodtext[FlevelNumber].ModificationNote +'</p>';		
			}	
			if (FlevelNumber[0] == "4") {
				htmlString += '<p>' + "IDDSI level " + FlevelNumber + " " + FDes + " has no lumps; doesn't require chewing or biting. <br />" + foodtext[FlevelNumber].ModificationNote +'</p>';		
			}	
			if (FlevelNumber[0] == "5") {
				htmlString += '<p>' + "IDDSI level " + FlevelNumber + " " + FDes + " must be soft and moist; requires some chewing but not biting. <br />" + foodtext[FlevelNumber].ModificationNote +'</p>';		
			}
			if (FlevelNumber[0] == "6") {
				htmlString += '<p>' + "IDDSI level " + FlevelNumber + " " + FDes + " must be soft, tender and moist; no separate thin liquids; requires chewing but no biting. <br />" + foodtext[FlevelNumber].ModificationNote +'</p>';		
			}	
			if (FlevelNumber[0] == "7") {
				htmlString += '<p>' + "IDDSI level " + FlevelNumber + " " + FDes + " must be soft with tender texture; can have liquids, but not thin liquid; requires chewing and biting. <br />" + foodtext[FlevelNumber].ModificationNote +'</p>';
			...
			htmlString += '<p>' + "To qualify for IDDSI level " + FlevelNumber + " " + FDes + ", your " + carriedDFValue + " item must pass the following test(s). <br /> See video guide below for details" + '.</p>';
		}
...
// 			alert ("Got to Test Section-Drink")	
			...
			if (DlevelNumber[0] == "4") {
			document.getElementById('testFD').innerHTML = "Fork Drip Test: Item that are " + foodtext[DlevelNumber].ForkDripDescription + ".";
...
// 			alert ("Got to Test Section-Food")
	...
	// 		document.getElementById('testFP').innerHTML = "Fork Pressure Test: Food that are " + foodtext[FlevelNumber].ForkPressureTest + ".";
			...
			if (FlevelNumber[0] == "5") {
			document.getElementById('testSi').innerHTML = "Size: No more than " + foodtext[FlevelNumber].BitingParticleSize + ".";
...
// 				alert ("Got to the Food portion")
				...
				if (carriedUI6 == "IDDSI 5") {
				document.getElementById('testSi').innerHTML = "Size: No more than " + foodtext[TlevelNumber].BitingParticleSize + ".";
